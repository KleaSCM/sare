...
impl TuiManager {
	...
	/**
	 * メインシェルインターフェースをレンダリングする関数です
	 * 
	 * ratatuiを使用してターミナルユーザーインターフェースを
	 * 描画し、シェルの状態を視覚的に表示します。
	 * 
	 * 上部バー（ユーザー名と作業ディレクトリ）、出力エリア、
	 * 入力エリアの3つのセクションに分割してレイアウトし、
	 * 各セクションに適切なスタイルとボーダーを適用します。
	 * 
	 * LineとSpanを使用してテキストを適切にフォーマットし、
	 * 色付きの表示でユーザビリティを向上させます。
	 * 
	...
	pub fn render(
		...
		let output_area = Paragraph::new(output_lines.iter().map(|line| {
			Line::from(vec![Span::raw(line)])
		}).collect::<Vec<_>>())
		.block(Block::default().borders(Borders::ALL).title("Output"));
		frame.render_widget(output_area, chunks[1]);
		
		let input_area = Paragraph::new(vec![Line::from(vec![Span::raw(input_buffer)])])
	 ...
	 * 
	 * @param output - Output text to add
	 */
	 ...
	 * 
	 * @return &[String] - Output history slice
	 */
