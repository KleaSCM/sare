...
impl NavigationManager {
	...
	pub fn new(config: NavigationConfig) -> Self {
		/**
		 * ナビゲーションマネージャーを初期化する関数です
		 * 
		 * ペイン間のナビゲーションを管理するための
		 * ナビゲーションマネージャーを作成し、デフォルトの
		 * ショートカットとフォーカス履歴を設定します。
		 * 
		 * Ctrl+h/l/k/jなどのデフォルトショートカットを設定し、
		 * フォーカス履歴とナビゲーションモードを初期化します。
		 * 各ショートカットは適切なNavigationActionにマッピングされます。
		 */
	...
	pub async fn handle_navigation(&mut self, input: &str, panes: &[Pane], current_focus: Option<&str>) -> Result<NavigationResult> {
		 ...
		 * ナビゲーション入力を処理する関数です
		 * 
		 * 入力がショートカットかどうかをチェックし、ショートカットの
		 * 場合は対応するアクションを実行します。
		 * 
		 * ショートカットでない場合は現在のナビゲーションモードに
		 * 応じて適切な処理を行います（Normal、Quick、Visual、Command）。
		 * 
		 * 各モードは異なる入力処理方法を持ち、適切なナビゲーション
		 * アクションを実行して結果を返します。
		 */
