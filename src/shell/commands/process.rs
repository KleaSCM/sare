...
/**
 * プロセスやジョブを終了するkillコマンドです
 * 
 * 指定されたプロセスIDまたはジョブIDのプロセスに
 * SIGTERMシグナルを送信して終了させます。
 * 
 * ジョブID（%で始まる）の場合はシェルのジョブ管理を
 * 使用し、プロセスIDの場合はlibc::kill()を直接使用します。
 * 
 * 無効なIDや存在しないプロセス/ジョブの場合はエラーを
 * 返します。成功時は終了したプロセス/ジョブの情報を
 * 出力します。
 */
